...
class ListEntry extends MarginContainer:
	...
	var button_clear_instances: TextureButton
	var spacer: Control
	...
	@onready var clear_instances_icon: Texture2D = get_theme_icon("Remove", "EditorIcons")
	...
	func setup_buttons() -> void:
		...
		if type == Terrain3DAssets.TYPE_MESH:
			button_clear_instances = TextureButton.new()
			button_clear_instances.set_texture_normal(clear_instances_icon)
			button_clear_instances.set_custom_minimum_size(icon_size)
			button_clear_instances.set_h_size_flags(Control.SIZE_SHRINK_END)
			button_clear_instances.set_visible(resource != null)
			button_clear_instances.tooltip_text = "Clear All Instances"
			button_clear_instances.mouse_filter = Control.MOUSE_FILTER_PASS
			button_clear_instances.mouse_default_cursor_shape = Control.CURSOR_POINTING_HAND
			button_clear_instances.pressed.connect(_on_clear_instances)
			button_row.add_child(button_clear_instances, true)
	...
	func destroy_buttons() -> void:
		...
		if button_edit:
			button_edit.free()
			button_edit = null
		if button_clear_instances:
			button_clear_instances.free()
			button_clear_instances = null
		if spacer:
			spacer.free()
			spacer = null
		if button_clear:
	...
	func _on_clear_instances() -> void:
		if resource and resource is Terrain3DMeshAsset:
			var asset_dock: Control = get_parent().get_parent().get_parent().get_parent()
			asset_dock.confirm_dialog.dialog_text = "Clear all instances of '%s'?" % get_resource_name()
			asset_dock.confirm_dialog.popup_centered()
			await asset_dock.confirmation_closed
			if not asset_dock._confirmed:
				return
			asset_dock.plugin.terrain.instancer.clear_by_mesh(resource.id)
			update_count_label()
